...
from models.models import cursor_data, original_volume_data, changed_volume_data, toolframe_data
...
def image_volume_loader(volume, order=0, cube_side=200):
        ...
        original_volume_data.image_volume = volume
        point = (np.array(volume.shape[-1:-4:-1][::-1])/2).astype(int)
        cursor_data.current_point_original_vol = point
        multichannel = len(volume) > 3
        if(multichannel):
            toolframe_data.channel_intensity = list(volume[x, point[0], point[1], point[2]] for x in range(4))
            original_volume_data.num_of_channels = volume.shape[3]
        else:
            toolframe_data.channel_intensity = [volume[point[0], point[1], point[2]]]
            original_volume_data.num_of_channels = 1
        sides = np.array(list(volume.shape))
        max_side = sides.max()
        changed_volume_data.zoom_factors = (cube_side/max_side, cube_side/max_side, cube_side/max_side)
        if multichannel:
            volume = multi_channel_zoom(volume, changed_volume_data.zoom_factors, order=order)
        else:
            volume = zoom(volume, changed_volume_data.zoom_factors, order=order)
        volume = ((volume - volume.min())*(255/(volume.max() - volume.min()))).astype(np.uint8)
        changed_volume_data.changed_image_volume = volume  
        
def mask_volume_loader(mask):
    mask_zoom = changed_volume_data.zoom_factors
    original_volume_data.mask_volume = mask
    ...
    changed_volume_data.changed_mask_volume = RGBA_mask
